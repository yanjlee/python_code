...
for sym in symbol_list:
    ...
    if sym.startswith('6'):
        sym = '\'' + sym + '.SH\''
        price_data = get_data('date, close', 'stk_price_forward', sym, start_year, end_year)
    elif sym.startswith('0') or sym.startswith('3'):
        sym = '\'' + sym + '.SZ\''
        price_data = get_data('date, close', 'stk_price_forward', sym, start_year, end_year)
    elif sym.startswith('1') or sym.startswith('5'):
        sym = '\'' + sym + '.OF\''
        price_data = get_data('date, close', 'etf_price', sym, start_year, end_year)
    # if sym.startswith('SHSE'):
    #     sym =  sym.replace('SHSE.','\'') + '.SH\''
    # else:
    #     sym =  sym.replace('SZSE.','\'') + '.SZ\''
    # price_data  = get_data('date, close', 'stk_price_forward', sym, start_year, end_year)
    df_data = pd.DataFrame(list(price_data),columns=['datetime','close'])
...
#     df_data = get_k(sym, 60, 0, start_year, end_year)
    ...
    df_data.loc[:, 'chg'] = (df_data['close'] - df_data['close'].shift(1)) / df_data['close'].shift(N)
    df_data.loc[:, 'ret'] = (df_data['close'] - df_data['close'].shift(1)) / df_data['close'].shift(1)
    df_data.chg = df_data.chg.astype(float).round(5)
    df_data.ret = df_data.ret.astype(float).round(5)
    # df_idx = copy.copy(df_i)
    # df_return = copy.copy(df_ret)
    df_idx = pd.concat([df_idx, copy.copy(df_data.set_index('datetime').drop(['close','ret'],axis=1,inplace=False) \
                                          .rename(columns={'chg':sym.replace('\'','')}))],sort=True, axis=1).fillna(-1)
    df_return = pd.concat([df_return, copy.copy(df_data.set_index('datetime').drop(['close','chg'],axis=1,inplace=False)\
                                             .rename(columns={'ret':sym.replace('\'','')}))],sort=True, axis=1).fillna(0)
...
re, mdd, df_r = Run(df_idx,df_return,S/100)
