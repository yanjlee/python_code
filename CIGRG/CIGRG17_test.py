...
for i in range(1, length):
    ...
    if signal[i] == 1 and i_data.index[i][:8] not in future_change_date and holding[-1] == 1:
        ...
        # Profit.append(pro)
        print('铁矿石多头继续持仓 ' + str((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2))
        continue
    ...
    if signal[i] == -1 and i_data.index[i][:8] not in future_change_date and holding[-1] == -1:
        ...
        pro.append(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2)
        # Profit.append(pro)
        print('铁矿石空头继续持仓 ' + str(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2))
        continue

    if signal[i] == 0 and i_data.index[i][:8] not in future_change_date and holding[-1] == 1:
        holding.append(1)
        print('当日价格 ' + str(i_data.iloc[i, 0]) + ' 前日价格 ' + str(i_data.iloc[i - 1, 0]) + ' 开仓价格 ' + str(open_price[
                                                                                                            -1]))
        open_price.append(open_price[-1])
        pro.append((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2)
        # Profit.append(pro)
        print('多头继续持仓 ' + str((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2))
        continue
    ...
    if signal[i] == 0 and i_data.index[i][:8] not in future_change_date and holding[-1] == -1:
        ...
        pro.append(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2)
        # Profit.append(pro)
        print('空头继续持仓 ' + str(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2))
        continue
    ...
    if i_data.iloc[i, 0] > mean_10[i] and (
        ...
        print("铁矿石多单平仓出场 " + str((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2 - fee))
        continue
    if i_data.iloc[i, 0] < mean_10[i] and (
            i_data.iloc[i - 1, 0] < mean_30[i - 1] and i_data.iloc[i, 0] > mean_30[i]) and i_data.index[i][
                                                                                           :8] not in future_change_date and \
        ...
        # Profit.append(pro)
        trade_num = trade_num + 1
        print("铁矿石空单平仓 " + str(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2 - fee))
        continue
    ...
    if i_data.index[i][:8] not in future_change_date and holding[-1] == 1 and i_data.iloc[i, 0] < open_price[
        ...
        # Profit.append(pro)
        trade_num = trade_num + 1
        print("铁矿石多单止损出场 " + str((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2 - fee))
    if i_data.index[i][:8] not in future_change_date and holding[-1] == 1 and i_data.iloc[i, 0] > open_price[
        -1] * 1.04:
        print('当日价格 ' + str(i_data.iloc[i, 0]) + ' 前日价格 ' + str(i_data.iloc[i - 1, 0]) + ' 开仓价格 ' + str(open_price[
                                                                                                            -1]))
        ...
        # Profit.append(pro)
        trade_num = trade_num + 1
        print("铁矿石多单止盈出场 " + str((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2 - fee))
        continue
    if i_data.index[i][:8] not in future_change_date and holding[-1] == -1 and i_data.iloc[i, 0] > open_price[
        -1] * 1.02:
        holding.append(0)
        ...
        trade_num = trade_num + 1
        # Profit.append(pro)
        print("铁矿石空单止损失出场 " + str(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2 - fee))
        continue
    if i_data.index[i][:8] not in future_change_date and holding[-1] == -1 and i_data.iloc[i, 0] < open_price[
        -1] * 0.96:
        holding.append(0)
        ...
        # Profit.append(pro)
        trade_num = trade_num + 1
        print("铁矿石空单止盈出场 " + str(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2 - fee))
        continue
    ...
    if i_data.index[i][:8] in future_change_date:
        ...
        if holding[-1] == 1:
            ...
            pro.append((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2)
            trade_num = trade_num + 1
            print("铁矿石多头换月 " + str((i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2))
        ...
        if holding[-1] == -1:
            ...
            pro.append(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2)
            trade_num = trade_num + 1
            print("铁矿石空头即将换月 " + str(-(i_data.iloc[i, 0] - i_data.iloc[i - 1, 0]) * unit * 2))

    else:
        print(holding[i])
        print(signal[i])
        print(open_price[-1])
        print(i_data.iloc[i, 0])
        holding.append(0)
        open_price.append(0)
        pro.append(0)
        print("没有考虑到的情况")
