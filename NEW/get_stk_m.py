...
def Run(cci_n, rsi_n, k_data):
    ...
    pre_rsi_list = list(df[rsi_col].iloc[1])
    pre_cci_list = list(df[cci_col].iloc[1])
...
# symbol_list = ['SHSE.510880','SZSE.159901','SZSE.159915','SHSE.510500','SHSE.518880','SZSE.159919','SHSE.510900','SHSE.511260','SHSE.513500','SHSE.510050']
...
for n_year in range(0, 1):
    ...
    # 查询历史行情
        ...
        # DrawSignals2(k_data)
        # print([sym,start_year.strftime('%Y-%m-%d'),end_year.strftime('%Y-%m-%d'), re, mdd])
        print([sym, start_year, end_year, re, mdd])

        # print(str(k_data.datetime.iloc[0]) + ' ~ ' + str(k_data.datetime.iloc[-1]))
        # total_return.append([sym,start_year.strftime('%Y-%m-%d'),end_year.strftime('%Y-%m-%d'), re, mdd])
    ...
    # return_m.append([start_year, end_year, return_met['return'].mean(), ret['mdd'].mean(),(ret['return'].mean() - 1) / -ret['mdd'].mean()])
#
#
#
#
