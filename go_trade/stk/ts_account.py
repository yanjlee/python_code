...
def update_info():
    ...
    if len(trades) > 0 :
        for j in range(0, len(trades)):
            update_act_td = 'insert into act_td values(' + str(trades['成交编号'].iloc[j]) + ',' + trades['合同编号'].iloc[j] + ',\'' + dt.isoformat(dt.strptime(str(trades['成交日期'].iloc[j]),'%Y%m%d'))[:10] + '\',\'' + trades['操作'].iloc[j] + '\',\'' + trades['证券代码'].iloc[j] + '\',\'' + trades['证券名称'].iloc[j] + '\','+ str(trades['成交均价'].iloc[j]) + ','+ str(trades['成交数量'].iloc[i]) + ','+ str(trades['成交金额'].iloc[i]) + ');'
            try:
